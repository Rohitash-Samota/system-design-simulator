...
import { capacitySentence, formatRate } from "@/lib/format";
    ...
    <div className="flex h-full flex-col">
      <div className="shrink-0 px-3 pt-3 pb-2">
        ...
        {query ? (
          ...
          <p className="mt-2 flex items-center justify-between text-[10px] text-zinc-400">
            <span>Drag onto the canvas to build</span>
            <span className="tabular-nums">max req/sec</span>
          </p>
        )}
      </div>
          ...
          return (
            <div key={cat.key}>
              <div className="mb-2 flex items-center gap-2">
                <p className="bp-label !text-[9px] text-zinc-400">{cat.label}</p>
                <span className="flex h-4 min-w-4 items-center justify-center rounded-[2px] bg-zinc-800 px-1 font-mono text-[10px] font-medium tabular-nums text-zinc-400">
                  {items.length}
                </span>
                ...
                {items.map((item) => {
                  ...
                  return (
                    <Tooltip key={item.id}>
                        ...
                        render={
                          ...
                          >
                            ...
                            <span
                              ...
                              aria-label={capacitySentence(item.maxQPS)}
                            >
                              {formatRate(item.maxQPS)}
                            </span>
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                      ...
                      />
                      <TooltipContent side="right" sideOffset={8} className="max-w-[240px] flex-col items-start gap-1.5">
                        <p className="text-xs leading-relaxed">{tipText}</p>
                        <p className="text-[11px] leading-relaxed opacity-70">
                          {capacitySentence(item.maxQPS)}
                        </p>
                      </TooltipContent>
                    </Tooltip>
                  );

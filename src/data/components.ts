...
export const SYSTEM_COMPONENTS: SystemComponent[] = [
  ...
  },
  {
    ...
    stateful: true,
    description:
      "Streams row-level changes out of a database by tailing its replication log (Postgres WAL, MySQL binlog) rather than polling tables, so downstream caches, search indexes, and warehouses stay in sync without dual writes. This is the standard answer to keeping a search index consistent with the primary store. Debezium, AWS DMS, and Kafka Connect are the common tools.",
  },
  // Compute
  {
